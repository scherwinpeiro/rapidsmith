...
public class DeviceBrowser extends QMainWindow{
	...
	public DeviceBrowser(QWidget parent){
		...
		List<String> parts = DeviceDatabaseProvider.getDeviceDatabase().getAvailableParts();
		if(parts.size() < 1){
		...
		device = DeviceDatabaseProvider.getDeviceDatabase().loadDevice(currPart);
		we = DeviceDatabaseProvider.getDeviceDatabase().loadWireEnumerator(currPart);
