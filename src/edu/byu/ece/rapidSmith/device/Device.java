...
public class Device implements Serializable{
	...
	public static Device getInstance(String partName){
		//TODO keep multiple singletons in memory...
	...
	public Integer getPrimitiveExternalPin(Pin pin){
		return getPrimitiveExternalPin(pin.getInstance(), pin.getName());
	}

	/**
	 * Gets the external wire enumeration on the instance pin.
	 * @param instance The instance to use
	 * @param pinName The pin name to get the external name from.
	 * @return The wire enumeration of the internal pin on the instance primitive of pin.
	 */
	public Integer getPrimitiveExternalPin(Instance instance, String pinName){
		// Check the Pin Mapping patch for proper external pin name mapping
		String extName = PinMappingPatch.getPinMapping(instance.getType(), pinName);
		if(extName != null){
			return WireEnumerator.getInstance(getFamilyType()).getWireEnum(extName);
		}

		Integer extPin = instance.getPrimitiveSite().getExternalPinWireEnum(pinName);
		if(extPin != null){
			return extPin;
		}
...
//		
	...
	private void createCompatibleSiteIndex(){
		...
		for(PrimitiveType type: PrimitiveType.values()){
			...
			// Check for other compatible site types
			PrimitiveType[] compatibleTypes = type.getCompatibleTypes(this);
			if(compatibleTypes != null){
				for(PrimitiveType compatibleType : compatibleTypes){
					match = getAllPrimitiveSitesOfType(compatibleType);
					if(match != null){
...
//System.out.println("Wire:"+we.getWireName(wire));
							...
							for (WireConnection w : tile.getWireHashMap().get(wire)) {
								if(w.getColumnOffset() != 0 || w.getRowOffset() != 0){
									...
									watchDog = 0;
									while (!tileStack.isEmpty() && watchDog < 1000) {   //100
										watchDog++;
...
//System.out.println("  W1:"+we.getWireName(w1.getWire()));
										for (WireConnection wire2 : connections) {

//System.out.println("    Wire2:"+we.getWireName(wire2.getWire()));
//System.out.println("Wire: "+we.getWireName(wire2.getWire())+"  Rowoffset: "+wire2.getRowOffset()+"  ColOffset: "+wire2.getColumnOffset());

//					if(we.getWireName(wire2.getWire()).contains("CLK") && !switchMatrixTileTypes.contains(wire2.getTile(t1).getType()))
//						System.out.println("found CLK (wire2) on Tile "+wire2.getTile(t1).getName());
//					if(we.getWireName(w1.getWire()).contains("CLK") && !switchMatrixTileTypes.contains(t1.getType()))
//
											System.out.println("found CLK (w1) on Tile "+t1.getName());
//================================================================
// added for Spartan6 adaption
//----------------------------------------------------------------					
											if(setOfExternalPrimitivePins.contains(wire2.getWire()) && !switchMatrixTileTypes.contains(wire2.getTile(t1).getType())){
//================================================================
...
//												System.out.println(xOffset+" "+yOffset+"   Tile "+tile.getName()+" x:"+tile.getColumn()+" y:"+tile.getRow()+" Tile1 "+wire2.getTile(t1).getName()+"    t1:"+t1.getName()+" x:"+wire2.getTile(t1).getColumn()+" y:"+wire2.getTile(t1).getRow()+"  Wire "+we.getWireName(wire2.getWire()));
												if(found == null){
...
//													System.out.println("skip!  watchdog:"+watchDog);
													continue;
												}
...
// added for Spartan6 adaption
//----------------------------------------------------------------
											else if(wire2.getTile(t1) != null && (!switchMatrixTileTypes.contains(wire2.getTile(t1).getType()) || we.getWireName(wire2.getWire()).contains("CLK"))){
//================================================================
	...
	public boolean readDeviceFromCompactFile(String fileName){
		...
		try {
			return readDeviceFromCompactInputStream(new FileInputStream(fileName), fileName);
	 ...
	 * This reads from the compact device file to populate all of the essential variables of this
	 * device.  It can only be used to read files generated with the writeDeviceToCompactFile() method.
	 * @param is The compact file input
	 * @param fileName file name (for family detection)
	 * @return True if operation was successful, false otherwise.
	 */
	public boolean readDeviceFromCompactInputStream(InputStream is, String fileName){
		try {
