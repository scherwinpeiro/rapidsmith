...
public class FileConverter {
	...
	public static boolean convertNCD2XDL(String ncdFileName, String xdlFileName) {
		return convertXDL2NCD(Paths.get(ncdFileName), Paths.get(xdlFileName));
	}
	 ...
	 */
	public static boolean convertNCD2XDL(Path ncdFileName, Path xdlFileName){
		// Generate XDL
		ProcessBuilder pb = new ProcessBuilder("xdl", "-ncd2xdl", ncdFileName.toAbsolutePath().toString(), xdlFileName.toAbsolutePath().toString());
		try {
			Process p = pb.start();
			StreamGobbler input = new StreamGobbler(p.getInputStream(), false);
			StreamGobbler err = new StreamGobbler(p.getErrorStream(), false);
	...
	public static boolean convertXDL2NCD(Path xdlFileName, Path ncdFileName){
		return convertXDL2NCD(xdlFileName, ncdFileName, false);
	}
	...
	public static boolean convertXDL2NCD(String xdlFileName, String ncdFileName, boolean useLegacyTools){
		return convertXDL2NCD(Paths.get(xdlFileName), Paths.get(ncdFileName), useLegacyTools);
	}
	 ...
	 * @return True if operation was successful, false otherwise.
	 */
	public static boolean convertXDL2NCD(Path xdlFileName, Path ncdFileName, boolean useLegacyTools){
		String xdlPath = "xdl";
		if(useLegacyTools){
			xdlPath = RunXilinxTools.getBinPathToLegacyXilinxTools() + File.separator + xdlPath;
		}
		ProcessBuilder pb = new ProcessBuilder(xdlPath, "-xdl2ncd", "-force", xdlFileName.toAbsolutePath().toString(), ncdFileName.toAbsolutePath().toString());
		// Generate NCD
		try {
			Process p = pb.start();
