 ...
 *
 * OPTION :<br>
 *   -&lt;character&gt; specifies a short-form option <br>
 *   --&lt;string&gt; specifies a long-form option, separate word with '-' <br>
 *   Options are separated by a space
 *
 * <br>
 *
 * $ and ? characters indicate if the options expect accompanying values <br>
 * If a $ or ? + exists, the parser will look for a value to accompany the option <br>
 * If no $ or ? exists in the rule, no value accompanies the rule
 *
 * <br>
 *
 * $ indicates that the option expects an associated value with it. <br>
 *    Another occurrence of the option will result in an InvalidArgumentException <br>
 *    getOption() will return a String object of the value <br>
 * ? indicates that the option may optionally have an associated value. <br>
 *    getOption() will return a String object of the value or null if <br>
 *    it is not specified <br>
 *
 * <br>
 *
 * Examples : <br>
 * "-a" - an short-option with no value <br>
 * "-a --aardvark" - a short and long version of the same option <br>
 * "-a $" - a short-optoin with a single accompanying value <br>
 * Created on: Jun 23, 2012 <br>
 */
