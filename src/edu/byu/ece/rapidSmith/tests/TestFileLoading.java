...
public class TestFileLoading{
	...
	protected static boolean createTestBashScript(String fileName){
		try{
			...
			for(String partName : DeviceDatabaseProvider.getDeviceDatabase().getAvailableParts()){
	...
	public static void main(String[] args){
		...
		if(args[0].equals("-d")){
			dev = DeviceDatabaseProvider.getDeviceDatabase().loadDevice(args[1]);
		}
		else if(args[0].equals("-w")){
			we = DeviceDatabaseProvider.getDeviceDatabase().loadWireEnumerator(familyType);
		}
		else if(args[0].equals("-p")){
			list = DeviceDatabaseProvider.getDeviceDatabase().loadPrimitiveDefs(familyType);
		}
		...
		long fileSize = 0;
		if(args[0].equals("-d")){
			fileSize = new File(FileDeviceDatabase.getDeviceFileName(args[1])).length();
		}
		else if(args[0].equals("-w")){
			fileSize = new File(FileDeviceDatabase.getWireEnumeratorFileName(familyType)).length();
		}
		else if(args[0].equals("-p")){
			fileSize = new File(FileDeviceDatabase.getPrimitiveDefsFileName(familyType)).length();
		}
