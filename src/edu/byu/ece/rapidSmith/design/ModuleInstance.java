...
public class ModuleInstance{
	...
	private HashMap<Instance, Instance> inst2instMap;
	 ...
	 * @param placeImpossible
	 */
	public List<PrimitiveSite> getAllValidPlacements(BiConsumer<ModuleInstance, PrimitiveSite> placeImpossible){
		...
		for(PrimitiveSite newAnchorSite : sites){
			if(place(newAnchorSite, design.getDevice(), placeImpossible)){
		...
		if(originalSite != null)
			place(originalSite, design.getDevice(),placeImpossible);
		else unplace();
	 ...
	 */
	public boolean place(PrimitiveSite newAnchorSite, Device dev, BiConsumer<ModuleInstance,PrimitiveSite> placeImpossible){
		...
		for(Instance inst : instances){
			...
			if(newSite == null){
				placeImpossible.accept(this,templateSite);
		...
		for(Net net : nets){
			...
			if (templateNet==null)
				throw new RuntimeException("net "+net.getName()+" does not have a template");
			for(PIP pip : templateNet.getPIPs()){
				...
				if(newPipTile == null){
					unplace();
					//MessageGenerator.briefError("Warning: Routing tile for "+ name +" not found.");
					return false;
				}
				PIP newPip = new PIP(newPipTile, pip.getStartWire(), pip.getEndWire(), pip.getDirection());

				PIP newPipReverse = new PIP(newPipTile, pip.getEndWire(), pip.getStartWire());
				//TODO evaluate detecting invalid pips
				if(!newPipTile.hasPIP(newPip) && !newPipTile.hasPIP(newPipReverse)){
					//System.out.println("invalid pip: "+newPip);
					return false;
				}
				net.addPIP(newPip);
	...
	public HashMap<Instance, Instance> getInst2instMap() {
		return inst2instMap;
	}

	public void setInst2instMap(HashMap<Instance, Instance> inst2instMap) {
		this.inst2instMap = inst2instMap;
	}

	@Override
	public String toString() {
		return "ModuleInstance "+ name +" of "+module.getName();
	}

	/**
	 * Get a stream of all tiles used by nets
	 * @return
     */
	public Stream<Tile> streamRoutingTiles() {
		return nets.stream().flatMap((net) -> net.getPIPs().stream()).map(PIP::getTile);
	}

	/**
	 * Get a stream of all tiles used by instances or nets
	 * @return
     */
	public Stream<Tile> streamAllTiles() {
		return Stream.concat(instances.stream().map(Instance::getTile), streamRoutingTiles());
	}
}
