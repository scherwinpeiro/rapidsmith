...
public class Net implements Comparable<Net>, Serializable, WireContainer, JacksonDeserListener, INamed {
	 ...
	 */
	@JsonProperty("type")
	private NetType type = NetType.WIRE;
	/**
	...
	/**
	 * Return the opposite Pin: If supplied with the source of this net, return all sinks
	 * Otherwise, return the source
	 *
	 * @return
	 */
	public Stream<Pin> streamOppositePins(Pin pin) {
		if (pin == source) {
			return pins.stream().filter(p -> p != source);
		}
	...
	public void unroute() {
		...
		if (this.getSource() != null) {
			this.getSource().clearDelayCache();
		}
	}
	...
	/**
	 * If the net has exactly one sink, return it.
	 * Otherwise return null.
	 *
	 * @return The single sink or null if there is none or multiple
	 */
	public Pin findSingleSink() {
		//The number of pins in the net minus the source (if there is one) needs to be 1, otherwise there are multiple sinks
		if ((getSource() != null && pins.size() != 2)
				|| (getSource() == null && pins.size() != 1)) {
			return null;
		}
	...
	public void afterDeserialization() {
		for (Pin pin : pins) {
			pin.setNet(this);
			pin.getInstance().getNetList().add(this);
		}
	}

	public Net clone(Function<Pin, Pin> mapPin) {
		Net clone = new Net(name, type);
		if (attributes != null) {
		...
		clone.pips = pips.stream().map(PIP::clone).collect(Collectors.toList());
		if (source != null) {
			clone.source = mapPin.apply(source);
		}
		clone.fanOut = fanOut;
		clone.moduleInstance = moduleInstance;
		clone.moduleTemplate = moduleTemplate;
		clone.moduleTemplateNet = moduleTemplateNet;
		return clone;
	}
}
